...
const FinancialHealth = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h4 className="text-lg font-semibold font-headline mb-2">Arus Kas</h4>
        <div className="flex justify-between items-center mb-1 text-sm">
            <span className={netCashFlow >= 0 ? 'text-green-400' : 'text-red-400'}>Bersih: {formatCurrency(netCashFlow)}</span>
            <span className="text-muted-foreground">{formatCurrency(totalIncome)} Pemasukan</span>
        </div>
      ...
      <div>
        <h4 className="text-lg font-semibold font-headline mb-3">Wawasan Pengeluaran</h4>
        {isInsightsLoading ? (
          ...
          <ul className="space-y-3">
            {insights.map((insight, index) => (
             ...
             {insights.length === 0 && !isInsightsLoading && (
                 <p className="text-sm text-muted-foreground">Belum ada wawasan tersedia. Tambahkan lebih banyak transaksi!</p>
            )}
          </ul>
        )}
      </div>
